...
class HardAttn(nn.Module):
    def __init__(self, in_channels):
        ...
        # self.fc = nn.Sequential(
        self.fc1 = nn.Linear(128*128,32).apply(weights_init_kaiming)
        #init.kaiming_normal_(self.fc1.weight.data, a=0, mode='fan_in')
        #init.constant_(self.fc1.bias.data, 0.0)
        self.bn1 = nn.BatchNorm1d(32).apply(weights_init_kaiming)
        # print(self.bn1.weight.data)
        # print(self.bn1.bias.data)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(32,4).apply(weights_init_kaiming)
        #init.kaiming_normal_(self.fc2.weight.data, a=0, mode='fan_in')
        #init.constant_(self.fc2.bias.data, 0.0)
        self.bn2 = nn.BatchNorm1d(4).apply(weights_init_kaiming)
       #)#.apply(weights_init_kaiming)

    def forward(self, x):
        #print(self.fc[4].weight)
        global_feature=x
        stn = (global_feature.sum(1)/float(global_feature.size(1))).view(global_feature.size(0),-1)
        # print(stn)
        stn = self.fc1(stn)
        # print(stn)
        stn = self.bn1(stn)
        ...
        stn = self.fc2(stn)
        stn = self.bn2(stn)
        # print(stn_feature.size())
        # stn_theta=self.fc(stn)
        #print(stn)
       # print(stn_feature[1])
        #print(stn_theta)
       # print(stn_theta[1])
...
class ResNet(nn.Module):
    ...
    def stn(self, x, stn_theta):
        ...
        #print(stn_theta)
        #stn_theta = stn_theta.view(x.size(0), 2, 3)
        theta[:,0,2]=0#stn_theta[:,3]
        theta[:,0,0]=1#stn_theta[:,0] 
        theta[:,1,1]=0.6#stn_theta[:,1]
        theta[:,1,2]=-0.4#stn_theta[:,2]
        # print(theta.view(x.size(0), 6))
        grid = F.affine_grid(theta, torch.Size([x.size()[0], 3, 128, 128]))
        # grid = F.affine_grid(stn_theta, torch.Size([x.size()[0], 3, 128, 128]))
        # print(grid)
