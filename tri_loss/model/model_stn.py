...
from tri_loss.model.resnet_stn import resnet50, resnet152, resnet101
# from .resnet_stn import resnet50, resnet152, resnet101
import time
from tri_loss.utils.candidate_parts import generate_parts
# from ..utils.candidate_parts import generate_parts
import math
from torch.autograd import Variable
from tri_loss.model.layer import *
# from .layer import *
import numpy as np
...
class Model(nn.Module):
    def __init__(self, last_conv_stride=11, num_classes=13164, is_RPP=True, basenet='Resnet50', loss='Cosface', is_pool=True):
        ...
        if self.loss == "Softmax":
            self.id_classifier1 = nn.Linear(self.feature_in_dim, num_classes, bias=True)#.apply(weights_init_kaiming)
            self.id_classifier3 = nn.Linear(self.feature_in_dim, num_classes, bias=True)#.apply(weights_init_kaiming)
    ...
    def forward(self, img, target):
        ...
        #id_feat = id_feat1 + id_feat3

        return id_feat, id_logit1, id_logit3


if __name__ == '__main__':
    model = Model(last_conv_stride = 1, num_classes = 10, loss = "Softmax").cuda()
    np.random.seed(42)
    x = torch.from_numpy(np.random.randint(0, 255, size=(2, 3, 128, 128)).astype(np.float32)).cuda()
    theta = torch.from_numpy(np.random.rand(2, 6).astype(np.float32)).cuda()
    print(x)
    print(theta)
    # x = torch.from_numpy(np.ones((2, 3, 128, 128), dtype=np.float32)).cuda()
    feat, id1, id2 = model(x, [], theta)
    # print(id1, id2)
