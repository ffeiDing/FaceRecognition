...
from .resnet_cstn import resnet50, resnet152, resnet101
import time
...
class Model(nn.Module):
    def __init__(self, last_conv_stride=11, num_classes=13164, is_RPP=True, basenet='Resnet50', loss='Cosface', is_pool=True):
        ...
        if self.loss == "Softmax":
            ...
            self.mask_classifier = nn.Linear(self.feature_in_dim, 2, bias=False)#.apply(weights_init_kaiming)
            weights_init_classifier(self.mask_classifier)
            self.id_classifier_fuse = nn.Linear(self.feature_in_dim, num_classes, bias=False)#.apply(weights_init_kaiming)
            weights_init_classifier(self.id_classifier_fuse)
    ...
    def forward(self, img, target):
        ...
        if self.loss == "Softmax":
            ...
            id_logit1=self.id_classifier1(id_feat1)
            id_logit3=self.id_classifier3(id_feat3)
            mask_logit = self.mask_classifier(id_feat1)
            sm = nn.Softmax(dim=1)
            mask_logit_softmax = sm(mask_logit)
            
            #id_feat1 = id_feat1*mask_logit_softmax[:,0].view(-1,1).expand(id_feat1.size())
            #id_feat3 = id_feat3*mask_logit_softmax[:,1].view(-1,1).expand(id_feat1.size())
            
            id_feat_fuse = id_feat1+id_feat3*mask_logit_softmax[:,1].view(-1,1).expand(id_feat1.size())
            #id_feat_fuse = id_feat1*mask_logit_softmax[:,0].view(-1,1).expand(id_feat1.size())+id_feat3*mask_logit_softmax[:,1].view(-1,1).expand(id_feat1.size())
            
            #id_feat_fuse = torch.cat((id_feat1, id_feat3),1)
            #id_feat_fuse = id_feat1+id_feat3
            
            id_logit_fuse =self.id_classifier_fuse(id_feat_fuse)
            
        

        #return id_feat_fuse, id_logit1, id_logit3, [], mask_logit
        return id_feat3, id_logit1, id_logit3, id_logit_fuse, mask_logit
