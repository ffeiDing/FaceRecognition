...
class TestSet(Dataset):
  ...
  def extract_feat(self, normalize_feat, verbose=True):
    ...
    while not done:
      ...
      if flag == 1:
        flag = 0
        with open('im.npy', 'wb') as f1:
            np.save(f1, ims_)
        with open('feat.npy', 'wb') as f2:
            np.save(f2, np.array(feat1_))
    ...
    #print(im_names[5135])
    #print(im_names[5136])
    #with open('gallery_names.npy', 'wb') as f1:
    #    np.save(f1, im_names[5136:])
    #with open('gallery_feats.npy', 'wb') as f2:
    #    np.save(f2, feat1[5136:])
    # print(feat1, ids, cams, im_names, marks)
    if False:
        with open('single_feat.npy', 'wb') as f3:
            np.save(f3, feat1)
        sys.exit(0)
  ...
  def eval(
    ...
    def compute_score(
        ...
        query_names=im_names[q_inds],
        gallery_names=im_names[g_inds]):
      # Compute mean AP
      mAP = mean_ap(
        distmat=dist_mat,
        query_ids=query_ids, gallery_ids=gallery_ids,
        query_cams=query_cams, gallery_cams=gallery_cams, 
        query_names=query_names, gallery_names=gallery_names)
      # Compute CMC scores
      ...
      if False:
        for i in range(len(query_ids)):
          if q_names[i] == "/ssd_datasets/dff/1018/2_1/0_0.png":
            print("query:")
            print(q_names[i])
            for j in range(30):
                print(g_names[indices[i][j]])
    ...
    with measure_time('Computing distance...', verbose=verbose):
      ...
      q_g_dist_list=[]
      q_g_dist = compute_dist(feat1[q_inds], feat1[g_inds], type='cosine')
      print(q_g_dist)
