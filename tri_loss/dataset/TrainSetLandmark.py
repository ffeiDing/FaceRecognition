...
class TrainSetLandmark(Dataset):
  ...
  def __init__(
    ...
    print (kwargs)
    super(TrainSetLandmark, self).__init__(
  ...
  def get_sample(self, ptr):
    ...
    im_scales = [self.im_scales[ind] for ind in inds]
    # im_scales = [0 for ind in inds]
    ...
    return ims, im_names, labels, mirrored, mask_labels, im_scales
  ...
  def next_batch(self):
    ...
    im_list, im_names, labels, mirrored, mask_labels, im_scales = zip(*samples)
    # t = time.time()
    ...
    im_scales = np.concatenate(im_scales)
    labels = np.concatenate(labels)
    mirrored = np.concatenate(mirrored)
    mask_labels = np.concatenate(mask_labels)
    #print(mask_labels)
    return ims, im_names, labels, mirrored, self.epoch_done, mask_labels, im_scales
