...
def load_state_dict(model, src_state_dict):
    ...
    for name, param in src_state_dict.get('state_dicts')[0].items():
        ...
        name = name.replace("global_bn1", "global_bn")
        name = name.replace("global_bn3", "partial_bn")
        name = name.replace("id_classifier1", "global_fc")
        name = name.replace("id_classifier3", "partial_fc")
        if name.endswith(".bias") and ("bn" in name or ("down" in name and ".1" in name)):
            name = name[:name.rfind(".bias")]
            name = name + ".beta"
        elif name.endswith(".weight") and ("bn" in name or ("down" in name and ".1" in name)):
...
def adjust_lr_warmup(optimizer, base_lr, ep, decay_at_epochs, factor):
    assert ep >= 1, "Current epoch number should be >= 1"
    if ep < decay_at_epochs[0]:
        for g in optimizer.param_groups:
            g['lr'] = (base_lr) *(float(ep+1)/float(decay_at_epochs[0]))

        print('=====> lr adjusted to {:.10f}'.format(g['lr']))
        return
