            ...
            if len(dets) != 1:
                ...
                for face in dets:
                    ...
                    if len(shape.parts())==0:
                        ...
                        landmark = shape.parts()[29].y
                        if landmark < img.shape[0]/3.0:
                            scale = 0.6
                        else:
                            scale = 1.0*landmark/img.shape[0]
            print(scale)                             
            new_trainval_im_scales.append(scale)

            mask_name_path = os.path.join(random.choice(mask_paths_list), id_name, img_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_dataset_path', type=str, default="/ssd_datasets/web_face/ourself_dataset/MTCNN_align_600id")
    args = parser.parse_args()
    test_dataset_path = args.test_dataset_path
    train_ori_dataset_path = args.train_ori_dataset_path
    partition_path = osp.join(train_ori_dataset_path, "mask_landmark.pkl")
