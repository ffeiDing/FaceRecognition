...
from tri_loss.model.model_lpd import Model
...
class Config(object):
    def __init__(self):
        ...
        parser.add_argument('--partition_path', type=str, default='/home/dff/MFR/FaceRecognition/script/pkl/test_mask_normal.pkl')
        parser.add_argument('--test_path', type=str, default='/home/dff/Attribute/web_tmp/0_0.png')
        # parser.add_argument('--partition_path', type=str, default='/ssd_datasets/dff/1018/test_1018_partition.pkl')#'/home/dff/f/data/faces_webface_112x112_raw_image/mask_landmark.pkl')
        ...
        parser.add_argument('--rotate_prob', type=float, default=0.0)
        ...
        parser.add_argument('--exp_dir', type=str, default='/home/dff/MFR/FaceRecognition/logs/webface/lpd')
        parser.add_argument('--model_weight_file', type=str, default='/home/dff/MFR/FaceRecognition/logs/webface/lpd/ckpt_best.pth')
...
def main():
    ...
    for ep in range(start_ep, cfg.total_epochs):
        ...
        while not epoch_done:
            step += 1
            # step_st = time.time()
            ims, im_names, labels, mirrored,  epoch_done, mask_labels, scale_labels = train_set.next_batch()
            # print(type(idxs))

            ims_var = Variable(TVT(torch.from_numpy(ims).float()))
            scale_labels = Variable(TVT(torch.from_numpy(scale_labels).float()))
            labels_t = TVT(torch.from_numpy(labels).long())
            labels_var = Variable(TVT(torch.from_numpy(labels).long()))
            
            feat, logit1, logit3, scale = model_w(ims_var, labels_var)

            loss1 = softmax_criterion(logit1, labels_var)
            loss2 = softmax_criterion(logit3, labels_var)
            loss3 = l1_loss_fn(scale, scale_labels)
