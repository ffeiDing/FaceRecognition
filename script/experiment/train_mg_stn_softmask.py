...
from tri_loss.model.model_stn_softmask import Model
...
class Config(object):
    def __init__(self):
        ...
        parser.add_argument('-d', '--sys_device_ids', type=eval, default=(4,))
        ...
        parser.add_argument('--exp_dir', type=str, default='/home/dff/MFR/FaceRecognition/logs/mg_stn_softmask_1')
        parser.add_argument('--model_weight_file', type=str, default='/home/dff/MFR/FaceRecognition/logs/mg_stn_softmask_1/ckpt.pth')
...
def main():
    ...
    for ep in range(start_ep, cfg.total_epochs):
        ...
        loss_softmax_meter4 = AverageMeter()
        ep_st = time.time()
        step_st = time.time()
        step = 0
        epoch_done = False
        ...
        while not epoch_done:
            ...
            mask_labels_var = Variable(TVT(torch.from_numpy(mask_labels).long()))
            
            feat, logit1, logit3, fuse_logit, mask_logit = model_w(ims_var, labels_var)

            loss1 = softmax_criterion(logit1, labels_var)
            loss2 = softmax_criterion(logit3, labels_var)
            loss3 = softmax_criterion(fuse_logit, labels_var)
            loss4 = softmax_criterion(mask_logit, mask_labels_var)
            ...
            loss_softmax_meter4.update(to_scalar(loss4))
            if step % cfg.steps_per_log == 0:
                time_log = '\tStep {}/Ep {}, {:.2f}s'.format(
                    step, ep + 1, time.time() - step_st, )
                
                tri_log = (', loss softmax1 {:.4f}, loss softmax2 {:.4f}, loss fuse {:.4f}, loss mask {:.4f}'.format(
                    loss_softmax_meter1.val, loss_softmax_meter2.val, loss_softmax_meter3.val, loss_softmax_meter4.val))
